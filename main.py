...
def get_user_memory(user_id: str) -> ConversationBufferWindowMemory:
    """Return memory for a given user, create if not exists."""
...
def parse_llm_response(llm_response: str) -> Dict[str, str]:
    try:
        # :one: Try to match properly formatted code block: ```json\n{...}\n```
        match = re.search(r'```(?:json)?\s*({[\s\S]*?})\s*```', llm_response, re.IGNORECASE)
        if not match:
            # :two: Fallback: match if the bot just printed "json\n{...}" without backticks
        ...
        if match:
            ...
            print(":warning: Still no JSON block found in LLM response.")
...
async def chat(
    ...
    user_id = str(user_info.get("user_id", "default"))
    memory = get_user_memory(user_id)
    if not isinstance(developer_list, list):
        return {
            "message": ":x: 'dev_names' must be a list of developer names.",
            "data": None,
            "complete": False,
        }
    if user_input.strip().lower() in ["yes", "confirm", "submit"] and is_form_complete(data_store):
        ...
        try:
            ...
            if res.get('status') == True:
                memory.chat_memory.add_ai_message(":white_check_mark: Estimate submitted.")
            ...
            return {
                "message": f":x: API call failed: {str(e)}",
                "data": None,
                "complete": False,
            }
    ...
    if is_form_complete(data_store) and not confirmation_received:
        ...
        memory.chat_memory.add_ai_message(f":clipboard: All fields collected:\n{summary}\n\nDo you want to submit this? (Yes/No)")
        return {
